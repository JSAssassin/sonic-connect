...
const addPlaylist = asyncErrorHandler(async (req, res, next) => {
  ...
  createResponse({ data: { playlist: newPlaylist }, res, statusCode: 201 });
});
...
const getPlaylist = asyncErrorHandler(async (req, res, next) => {
  ...
  if (!playlist) {
    const err = new CustomError(`Playlist with id "${id}" not found.`, 404);
    next(err);
  }
  createResponse({ data: { playlist }, res, statusCode: 200 });
});
...
const deletePlaylist = asyncErrorHandler(async (req, res, next) => {
  ...
  createResponse({ data: { playlist: null }, res, statusCode: 200 });
});
...
const updatePlaylist = asyncErrorHandler(async (req, res, next) => {
  ...
  createResponse({ data: { playlist: updatedPlaylist }, res, statusCode: 200 });
});
