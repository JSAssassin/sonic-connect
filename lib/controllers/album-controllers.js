...
const addAlbum = asyncErrorHandler(async (req, res, next) => {
  const { artists, featuredArtists, photo: photoId } = req.body;
  ...
  await album.populate(['artists', 'photo', 'featuredArtists']);
...
const getAlbums = asyncErrorHandler(async (req, res, next) => {
  ...
  const albums = await Album.find(query)
    .populate(['artists', 'photo', 'featuredArtists']);
