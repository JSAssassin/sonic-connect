...
const addPlaylist = asyncErrorHandler(async (req, res, next) => {
  ...
  await playlist.populate(['user', 'songs']);
...
const getPlaylists = asyncErrorHandler(async (req, res, next) => {
  ...
  const playlists = await Playlist.find(query).populate(['user', 'songs']);
  return createResponse({
...
const getPlaylist = asyncErrorHandler(async (req, res, next) => {
  ...
  const playlist = await Playlist.findById(id).populate(['user', 'songs']);
  if (!playlist) {
