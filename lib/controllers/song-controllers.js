...
const addSong = asyncErrorHandler(async (req, res, next) => {
  const { album: albumId, artists, featuredArtists, photo: photoId } = req.body;
...
const getSongs = asyncErrorHandler(async (req, res, next) => {
  const query = {};
  const { title, genre, artists, album } = req.query;
  if (title) {
...
const updateSong = asyncErrorHandler(async (req, res, next) => {
  ...
  const allowedFields = [
    ...
    'featuredArtists', 'photo'
  ];
  const { artists, album: albumId, featuredArtists, photo: photoId } = req.body;
  const bodyCopy = { ...req.body };
  if (artists) {
    const { uniqueArtists, e } = await validateArtistsExist(artists, 'artists');
    if (e) {
